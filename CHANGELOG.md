...
* **proxy/stats:** add per-request `cost_usd` to each entry in `/stats` → `recent_requests`, so tooling (e.g. a Claude Code statusline HUD) can attribute cost to a specific session by summing over `turn_id` / time window instead of shelling out to `ccusage`. The value is the billed estimate for that request (post-compression `input_tokens_optimized` + `output_tokens`, priced via the existing LiteLLM-backed `CostTracker`); it is `null` — never fabricated — when pricing is unavailable. Purely additive and backward-compatible; no new endpoints, headers, or config ([#1079](https://github.com/headroomlabs-ai/headroom/issues/1079)).

...
* **proxy:** make the off-path background compression executor configurable with `HEADROOM_BACKGROUND_COMPRESSION_WORKERS`, defaulting to the existing single-worker behavior while allowing high-concurrency deployments to drain cold-start compression work faster.

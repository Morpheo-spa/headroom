...
class KompressCompressor(Transform):
    ...
    def _ensure_model_loaded(self, *, allow_download: bool = True) -> tuple[Any, Any, str]:
        ...
        *not* at ``__init__`` time, keeping startup non-blocking.
        ...
        try:
            result = _load_kompress(
                model_id, self.config.device, allow_download=allow_download
            )
    ...
    def preload(self, *, allow_download: bool = True) -> str:
        ...
        Delegates to :meth:`_ensure_model_loaded` for the actual load.
        """

        _model, _tokenizer, backend = self._ensure_model_loaded(

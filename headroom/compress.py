...
class CompressConfig:
    ...
    Requires fastembed (already a Headroom dependency). Disable for zero overhead.
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if _env_semantic is not None:
            ...
            _score_enabled = _env_semantic.lower() not in ("false", "0", "no")
        else:
        ...
        if _score_enabled and tokens_saved > 0:
            try:
                ...
                if semantic_score_value is not None:
                    logger.debug(
                        "Semantic preservation score: %.3f "
